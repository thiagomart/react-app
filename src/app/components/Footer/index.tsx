...
export function Footer({terms}: Props): JSX.Element {
    ...
    <footer>
        <div className="mx-auto flex flex-col md:flex-row md:pt-[60px] sm:justify-between md:justify-start z-2">
            <hr className="border-foggy-grey mb-5 mt-5 md:hidden z-50" />
                <div className='sm:w-auto z-40'>
                    <button 
                        className="border-0 md:w-[119px] md:h-[70px]  sm:mb-0 sm:mr-4 w-full sm:w-auto text-[9px] flex 
                ...
                <div className="md:pl-5 w-full sm:w-auto md:pt-0 xs:pt-5 z-20">
                    <a href="https://verify.verifymyage.co.uk/ebay/signup" target="_blank" rel="noopener noreferrer" className=''>
                        <button className="border-2 border-black bg-white-smoke md:bg-white block rounded-full md:h-[70px] px-4 py-3.5 text-dark-grey shadow-sm ring-0 focus:ring-0 hover:ring-0 outline-none placeholder:text-gray-400 sm:text-sm sm:leading-6 w-full sm:w-auto z-10">
                            Got a VerifyMyAge account? Sign in
                        </button>
                    </a> 
                </div>
                <div className='md:hidden bg-white w-full h-[8.7rem] absolute z-0 right-0 bottom-[29px]'>
                </div>
            </div>
        </footer>
    );
